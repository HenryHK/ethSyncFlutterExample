...
//import 'item.dart';
//
//const double _kFlexibleSpaceMaxHeight = 256.0;   //main header depth   with design   appbar
//
...
//     // : assetName = 'assets/ethGreenHelp.png',
...
//    //sets the header style to colour from them
//    final TextStyle headerStyle = themeData.textTheme.body2.copyWith(color: Colors.red);     //themeData.accentColor);
//    String category;
...
//              title: const Text("ethSync main home ---"),                 //const Text('Flutter Gallery'),
...
//            child: const Banner(
//              message: 'PREVIEW 2 u load live ETC',
...
List<GalleryItem> _itemsWithCategory(String category) {
  return kAllGalleryItems
      .where((GalleryItem item) => item.category == category)
      .toList();
}
...
class GalleryHomeState extends State<GalleryHome>
  ...
  void initState() {
    ...
    _controller = new AnimationController(
      duration: const Duration(milliseconds: 600),
      debugLabel: 'preview banner', //make live ?
      vsync: this,
    )..forward();
  }
        ...
        body: new CustomScrollView(
          slivers: <Widget>[
            const SliverAppBar(
              ...
              expandedHeight: _kFlexibleSpaceMaxHeight,
              flexibleSpace: const FlexibleSpaceBar(
                title: const Text('ethSync'), //wider on android than ios
                // TODO(abarth): Wire up to the parallax in a way that doesn't pop during hero transition.
